...
- **ST/CT/WD 等编号** = **该项目**物料书代号，**不跨项目**当同一石材/同一单价；见 `docs/项目石材与主材编号价库.md`。
...
| **闭环校准** | `deliver` → 您校正 → `calibrate --learn` → `docs/闭环学习流程.md` |
| 外部价搜索 | `python app.py external-price` → `config/external_price_hints.json` |
...
```text
⓪ 项目主材编号（PT/ST/CT…）+ 工序分解（material_process_price）
        ↓
① 历史整项（名称+特征+做法达标）
        ↓
② 弱整项 / 金属线条 / 定制门家具
        ↓
③ 涂料工序拆价 paint_process（优先于平面市场参考）
        ↓
④ 市场参考价 market_reference
        ↓
...
| `docs/闭环学习流程.md` | **甲方→交付→校正→learn 闭环** |
